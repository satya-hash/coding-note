/* eslint-disable react/no-unescaped-entities */
import Link from "next/link";
import React from "react";

const page = () => {
  return (
    // <div className="latest_problems max-w-screen-lg mx-auto p-4 w-full">
    //   <div className="my-5">
    //     <h1 className="font-semibold md:text-2xl text-xl lg:text-3xl mb-3">
    //       About Satya Surendra
    //     </h1>
    //     <p className="text-base lg:text-lg">
    //       Welcome to Coding Note, where innovation meets expertise in front-end
    //       development and problem-solving. I%pos;m Pitani Satya Surendra, a
    //       dedicated computer science professional with a deep passion for
    //       crafting elegant and efficient software solutions.
    //     </p>
    //   </div>
    //   <div className="my-5">
    //     <h1 className="font-semibold md:text-2xl text-xl lg:text-3xl mb-3">
    //       Who I Am
    //     </h1>
    //     <p className="text-base lg:text-lg">
    //       I hold a Bachelor of Technology in Computer Science and Engineering
    //       from Andhra University, where I cultivated a strong foundation in both
    //       theoretical and practical aspects of computer science. My academic
    //       journey has fueled my curiosity and determination to excel in the tech
    //       industry.
    //     </p>
    //   </div>
    //   <div className="my-5">
    //     <h1 className="font-semibold md:text-2xl text-xl lg:text-3xl mb-3">
    //       My Professional Journey
    //     </h1>
    //     <p className="text-base lg:text-lg">
    //       Currently, I am a Member Technical at ADP, where I leverage my skills
    //       in Java full-stack development. My role involves both testing and
    //       developing software solutions, utilizing cutting-edge technologies
    //       such as Angular, TypeScript, Protractor, and Playwright. This hands-on
    //       experience has allowed me to contribute effectively to team projects,
    //       ensuring the delivery of high-quality software.
    //     </p>
    //   </div>
    //   <div className="my-5">
    //     <h1 className="font-semibold md:text-2xl text-xl lg:text-3xl mb-3">
    //       Skills and Expertise
    //     </h1>
    //     <table className="text-base lg:text-lg">
    //       <tr>
    //         <td>
    //           <strong>Front-end:</strong>
    //         </td>
    //         <td>Html,Css,Java script,React Js,Vite Js,Next Js,Scss</td>
    //       </tr>
    //       <tr>
    //         <td>
    //           <strong>Back-end:</strong>
    //         </td>
    //         <td>Java,Node Js,REST API,Spring Boot</td>
    //       </tr>
    //       <tr>
    //         <td>
    //           <strong>Testing & Automation:</strong>
    //         </td>
    //         <td>Protractor,PlayWritght</td>
    //       </tr>
    //       <tr>
    //         <td>
    //           <strong>Tools & Platforms:</strong>
    //         </td>
    //         <td>Git,Figma,Firebase</td>
    //       </tr>
    //       <tr>
    //         <td>
    //           <strong>Frameworks & Libraries:</strong>
    //         </td>
    //         <td>MUI,Rapid API,Commerce Js</td>
    //       </tr>
    //     </table>
    //   </div>
    //   <div className="my-5">
    //     <h1 className="font-semibold md:text-2xl text-xl lg:text-3xl mb-3">
    //       My Unique Value{" "}
    //     </h1>
    //     <p className="text-base lg:text-lg">
    //       At Coding Note, I bring a blend of creativity and analytical thinking
    //       to every project. I thrive in collaborative environments, working
    //       seamlessly with peers and mentors to achieve common goals. As a
    //       continuous learner, I am always staying current with industry trends
    //       and technologies to drive innovation and excellence.
    //     </p>
    //   </div>
    //   <div>
    //     <h1 className="font-semibold md:text-2xl text-xl lg:text-3xl mb-3">
    //       My Accomplishments{" "}
    //     </h1>
    //     <p className="text-base lg:text-lg">
    //       From academic projects to professional experiences, I have
    //       consistently demonstrated my ability to tackle complex problems and
    //       deliver impactful solutions. My notable projects include developing an
    //       e-commerce platform with React.js, MUI, and Commerce.js, a student
    //       management system with React.js and Firebase, and a user-friendly
    //       fitness app using React.js, MUI, and Rapid API.
    //     </p>
    //   </div>
    //   <div className="my-5">
    //     <h1 className="font-semibold md:text-2xl text-xl lg:text-3xl mb-3">
    //       Let's Connect{" "}
    //     </h1>
    //     <p className="text-base lg:text-lg">
    //       At Coding Note, I am always open to connecting with like-minded
    //       professionals and exploring new opportunities. Whether you're
    //       interested in discussing web development, Java projects, or potential
    //       collaborations, I'd love to connect and shape the future of technology
    //       together. Feel free to reach out to me at sathyagangadhara@gmail.com.
    //       Let's build something amazing!{" "}
    //     </p>
    //   </div>
    // </div>
    <div className="about max-w-screen-lg mx-auto p-4 w-full">
      <div className="my-5">
        <h1 className="font-semibold text-2xl lg:text-3xl mb-3 ">
          About Satya Surendra
        </h1>
        <hr className="w-[90%] bg-link-primary opacity-75 my-5" />
        <p className="text-base lg:text-lg">
          I'm Pitani Satya Surendra, a computer science professional passionate
          about front-end development and problem-solving. With a B.Tech in
          Computer Science from Andhra University and experience as a Member
          Technical at ADP, I specialize in creating efficient software
          solutions using the latest technologies.
        </p>
      </div>
      <div className="my-5">
        <h1 className="font-semibold text-2xl lg:text-3xl mb-3 ">
          Skills and Expertise
        </h1>
        <hr className="w-[90%] bg-link-primary opacity-75 my-5" />
        <table className="text-base lg:text-lg">
          <tr>
            <td>
              <strong>Front-end:</strong>
            </td>
            <td>Html,Css,Java script,React Js,Vite Js,Next Js,Scss</td>
          </tr>
          <tr>
            <td>
              <strong>Back-end:</strong>
            </td>
            <td>Java,Node Js,REST API,Spring Boot</td>
          </tr>
          <tr>
            <td>
              <strong>Testing & Automation:</strong>
            </td>
            <td>Protractor,PlayWritght</td>
          </tr>
          <tr>
            <td>
              <strong>Tools & Platforms:</strong>
            </td>
            <td>Git,Figma,Firebase</td>
          </tr>
          <tr>
            <td>
              <strong>Frameworks & Libraries:</strong>
            </td>
            <td>MUI,Rapid API,Commerce Js</td>
          </tr>
        </table>
      </div>
      <div className="my-5">
        <h1 className="font-semibold text-2xl lg:text-3xl mb-3 ">
          Let's Connect
        </h1>
        <hr className="w-[90%] bg-link-primary opacity-75 my-5" />
        <p className="text-base lg:text-lg">
          I'm always open to new opportunities and collaborations. Reach out to
          me at{" "}
          <Link
            href="email:sathyagangadhara@gmail.com"
            className="text-btn-primary"
          >
            sathyagangadhara@gmail.com
          </Link>
          . Let's create something amazing together!
        </p>
      </div>
    </div>
  );
};

export default page;
